...
{
	$url_utama = "http://localhost:8080/BreezyWash&Fold";
	if ($url != null) {
...
function order_ck($order_ck)
{
	...
	$jns_pkt = implode(", ", $order_ck['jenis_paket_ck']);
	$jenis_pkt_ck = htmlspecialchars($jenis_pkt_ck);
	$jns_pkt = htmlspecialchars($order_ck['jenis_paket_ck'][0]);
...
function order_cs($order_cs)
{
	...
	$jenis_pkt_cs = implode(", ", $order_cs['jenis_paket_cs']);
	$jenis_pkt_cs = htmlspecialchars($jenis_pkt_cs);
